...
interface VerificationResult
  extends Omit<ServiceVerificationResult, "document"> {
...
export default function VerifyDriverPage() {
  ...
  const searchMutation = useMutation({
    ...
    onSuccess: (data) => {
      // FIX: Convert all Date fields to ISO strings
      const safeDriver = {
        ...data,
        dateOfBirth: new Date(data.dateOfBirth).toISOString(),
        createdAt: new Date(data.createdAt).toISOString(),
        // Add any other Date fields
      };

      setDriverData(safeDriver);
  ...
  const verifyDocument = async (documentNumber: string) => {
    ...
    setIsVerifying(true);
    try {
      const result = await verificationService.verifyByDocumentNumber(
        documentNumber
      );
      ...
      const document = result.document
        ? {
            ...
            documentType:
              "documentType" in result.document
                ? String(result.document.documentType)
                : "DRIVER_LICENSE",
            expiryDate:
              "expiryDate" in result.document
                ? String(result.document.expiryDate)
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <Tabs
            value={searchType}
            onValueChange={(v: string) => setSearchType(v as "id" | "license")}
          >
            ...
            <form onSubmit={handleSearch} className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => router.push("/scan-qr")} // ← Next.js navigation
                >
              ...
              {driverData.documentNumber ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600">
                    No document information available for this driver
                  </p>
                </div>
              )}
            </CardContent>
