...
// Optional: Set token via API route (recommended for production)
async function setAuthCookie(accessToken: string, refreshToken: string) {
...
export default function LoginPage() {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: async (data) => {
      // Option 1: Secure - Set httpOnly cookie via API route
      try {
        await setAuthCookie(data.accessToken, data.refreshToken);
      } catch (err) {
        console.warn(
          "Failed to set httpOnly cookie, falling back to localStorage",
          err
        );
      }

      // Option 2: Fallback - Store in localStorage (for demo/dev)
        ...
        <div className="p-8">
          ...
          <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md text-sm">
            ...
            <div className="mt-1 space-y-1 text-yellow-700">
              ...
              <p>
                <span className="font-medium">Officer:</span> officer1 /
                officer123
              </p>
            </div>
          </div>
